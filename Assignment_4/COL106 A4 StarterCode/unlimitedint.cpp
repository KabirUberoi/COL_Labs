...
UnlimitedInt* UnlimitedInt::mul(UnlimitedInt* ini1, UnlimitedInt* ini2){
    ...
    for(int i=0; i<size1; i++){
        for(int j = size2-1; j>=0; j--){
            ...
            if(arr[i][j+1]>9){arr[i][j]+= (arr[i][j+1]/10); arr[i][j+1]%=10;}
        }
    }
    int answer[size1+size2+1];
    for(int i=0; i<(size1+size2+1); i++){
        answer[i]=0;
    }
    for(int i=0; i<size1; i++){
        for(int j=0; j<size2+1; j++){
            answer[i+j+1]+= arr[i][j];
        }
    }
    for(int i=(size1+size2); i>0; i--){
        if(answer[i]>9){answer[i-1]+=(answer[i]/10);answer[i]%=10;}
    }
    int count=0;
    for(int i = 0 ; i<(size1+size2+1); i++){
        if(answer[i]==0){count++;}
    ...
    if(itsSign==-1){ans+='-';}
    for(int i =count; i<(size1+size2+1); i++){
        ans += std::to_string(answer[i]);
    }
    UnlimitedInt* obj = new UnlimitedInt(ans);
    return obj;
}
